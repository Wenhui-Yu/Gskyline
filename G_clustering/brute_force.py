...
import random
import time
from distance_brute_force import distance_brute_force
import numpy as np
...
def brute_force(g_skyline, G_size, Dimension, k, iter_num):
    time1 = time.clock()
    cluster_list = []
    ...
    for i in range(k - 1):
        ...
        for index in range(len(g_skyline)):
            ...
            for cluster in cluster_list:
                ...
                G = np.array(G)
                [d, u] = distance_brute_force(G, cluster.center)
                dis *= d
            if dis > dis_rec:
                dis_rec = dis
                index_rec = index
    ...
    while (flag < 2 and num < iter_num):
        ...
        for group in g_skyline:
            ...
            for i in range(len(cluster_list)):
                ...
                [dis, u] = distance_brute_force(G, Gc)
                if (dis_cen > dis):
                    dis_cen = dis
                    u_cen = u
                    G_cen = Gc
